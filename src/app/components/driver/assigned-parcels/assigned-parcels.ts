...
export class AssignedParcels implements OnInit {
  ...
  startDelivery(parcelId: string) {
    ...
    this.parcelsService.updateParcelStatus(parcelId, {
      status: 'picked_up',
      notes: 'Parcel picked up by driver'
    }).subscribe({
      next: () => {
        // Then, start delivery (move to in_transit)
        this.parcelsService.updateParcelStatus(parcelId, {
          status: 'in_transit',
      ...
      error: (error) => {
        ...
        this.toastService.showError('Failed to pick up parcel');
      }
    });
  }

  completeDelivery(parcelId: string) {
    // First, mark as delivered to recipient
    this.parcelsService.updateParcelStatus(parcelId, {
      status: 'delivered_to_recipient',
      notes: 'Parcel delivered to recipient by driver'
    }).subscribe({
      next: () => {
        // Then, mark as delivered (completed by driver)
        this.parcelsService.updateParcelStatus(parcelId, {
          status: 'delivered',
      ...
      error: (error) => {
        ...
        this.toastService.showError('Failed to deliver to recipient');
      }
    });
  }
